...
describe("OpenAI provider account adapter (production-shaped store)", () => {
  it("lists a SEPARATE main row plus fallbacks, configured from the state store", () => {
    ...
    writeOpenAIState(dir, oaState({ fb1: { refresh: "rt-fb1", expires: 4_100_000_000_000 }, fb2: { refresh: "rt-fb2", expires: 4_100_000_000_000 } }));
  ...
  it("does NOT leak state-store credentials (separate store, no refresh in config)", () => {
    ...
    writeOpenAIState(dir, oaState({ main1: { refresh: "SECRET-MAIN", expires: 4_100_000_000_000 }, fb1: { refresh: "SECRET-FB1", expires: 4_100_000_000_000 } }));
  ...
  it("reports disabled/expired fallbacks without counting them configured", () => {
    ...
    writeOpenAIState(dir, oaState({ off: { refresh: "rt-off", expires: 4_100_000_000_000 }, fb1: { refresh: "rt-fb1", expires: 4_100_000_000_000 } }));
...
describe("Antigravity provider account adapter", () => {
  ...
  it("never leaks a raw refresh token, including email-less accounts", () => {
    const dir = tmpDir();
    writeAnti(dir, antiStore([{ email: undefined, refreshToken: "REAL-SECRET-TOKEN", addedAt: 1, lastUsed: 1, enabled: true }], 0));
    const accs = getAntigravityAccounts(dir);
  ...
  it("marks ineligible/verification accounts disabled (not configured)", () => {
    const dir = tmpDir();
    writeAnti(dir, antiStore([anti("a@x"), anti("b@x", { enabled: false }), anti("c@x", { accountIneligible: true })], 0));
    const accs = getAntigravityAccounts(dir);
...
describe("Unified manager-facing interface", () => {
  ...
  beforeEach(() => {
    ...
    writeOpenAIState(dir, oaState({ a: { refresh: "rt-a", expires: 4_100_000_000_000 }, b: { refresh: "rt-b", expires: 4_100_000_000_000 } }));
    writeAnti(dir, antiStore([anti("a@x"), anti("b@x")], 1));
  });
  ...
  it("reorderByManagerIds rejects ids spanning providers and duplicate permutations", async () => {
    const oaAll = getAccounts(dir).filter(a => a.provider === "openai").map(a => a.id);
    const anti = getAccounts(dir).filter(a => a.provider === "antigravity").map(a => a.id);
    ...
    const oaFallbacks = getOpenAIAccounts(dir).filter(a => !a.main).map(a => a.id);
...
describe("Manager account commands drive the real stores (async)", () => {
  ...
  beforeEach(() => {
    ...
    writeOpenAIState(configDir, oaState({ a: { refresh: "rt-a", expires: 4_100_000_000_000 }, b: { refresh: "rt-b", expires: 4_100_000_000_000 } }));
