...
describe("composeHooks preserves the full hook surface", () => {
  it("chains event-style hooks from every source", async () => {
    ...
    const hooks = composeHooks(
      {
        config: async () => {
          calls.push("openai-config");
        },
        "chat.headers": async () => {
          calls.push("openai-headers");
        },
        "command.execute.before": async () => {
          calls.push("openai-cmd");
        },
      },
      {
        config: async () => {
          calls.push("web-config");
        },
        "command.execute.before": async () => {
          calls.push("web-cmd");
        },
      ...
      {
        event: async () => {
          calls.push("fallback-event");
        },
        "tool.execute.after": async () => {
          calls.push("fallback-tool");
        },
      },
    );
  ...
  it("fails when multiple provider-scoped objects (auth) are composed", () => {
    expect(() =>
      composeHooks(
        { auth: { provider: "openai", methods: [] } as any },
        { auth: { provider: "antigravity", methods: [] } as any },
      ),
  ...
  it("merges tool registrations from every source and detects duplicate names", () => {
    ...
    expect(() => composeHooks({ tool: { dup: 1 as any } }, { tool: { dup: 2 as any } })).toThrow(
      /duplicate composed tool/i,
    );
  });
