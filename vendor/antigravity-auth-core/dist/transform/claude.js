 ...
 */
import { EMPTY_SCHEMA_PLACEHOLDER_DESCRIPTION, EMPTY_SCHEMA_PLACEHOLDER_NAME, } from "../constants.js";
...
export function appendClaudeThinkingHint(payload, hint = CLAUDE_INTERLEAVED_THINKING_HINT) {
    ...
    if (existing && typeof existing === 'object') {
        ...
        if (Array.isArray(sys.parts)) {
            const alreadyHasHint = sys.parts.some((p) => p &&
                typeof p === 'object' &&
                p.text === hint);
            if (alreadyHasHint)
                return;
...
export function normalizeClaudeTools(payload, cleanJSONSchema) {
    ...
    payload.tools.forEach((tool) => {
        ...
        if (Array.isArray(t.functionDeclarations) &&
            ...
            ;
            t.functionDeclarations.forEach((decl) => {
