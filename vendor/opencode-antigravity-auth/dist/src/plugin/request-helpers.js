...
function removeUnsupportedKeywords(schema, insideProperties = false) {
    ...
    for (const [key, value] of Object.entries(schema)) {
        if (!insideProperties &&
            UNSUPPORTED_KEYWORDS.includes(key)) {
            continue;
        }
        if (typeof value === 'object' && value !== null) {
            if (key === 'properties') {
...
function cleanupRequiredFields(schema) {
    ...
    if (Array.isArray(result.required) &&
        ...
        const validRequired = result.required.filter((req) => Object.hasOwn(result.properties, req));
...
export function extractThinkingConfig(requestPayload, rawGenerationConfig, extraBody) {
    ...
    if (thinkingConfig && typeof thinkingConfig === 'object') {
        ...
        return {
            ...
            thinkingBudget: typeof config.thinkingBudget === 'number'
    ...
    if (anthropicThinking && typeof anthropicThinking === 'object') {
        ...
        if (thinking.type === 'enabled' || thinking.budgetTokens) {
            return {
                ...
                thinkingBudget: typeof thinking.budgetTokens === 'number'
...
export function extractVariantThinkingConfig(providerOptions, generationConfig) {
    ...
    if (google) {
        ...
        if (google.googleSearch && typeof google.googleSearch === 'object') {
            ...
            result.googleSearch = {
                mode: search.mode === 'auto' || search.mode === 'off'
                    ? search.mode
                    : undefined,
    ...
    if (result.thinkingBudget === undefined &&
...
function transformGeminiCandidate(candidate) {
    ...
    const transformedParts = content.parts.map((part) => {
        ...
        if (part.type === 'thinking') {
            const thinkingText = typeof part.thinking === 'string'
                ? part.thinking
...
export function normalizeThinkingConfig(config) {
    ...
    const thinkingBudget = typeof budgetRaw === 'number' && Number.isFinite(budgetRaw)
        ? budgetRaw
        : undefined;
 ...
 */
export function createSyntheticTextResponse(text, extraHeaders = {}) {
...
export function createSyntheticErrorResponse(errorMessage, _requestedModel = 'unknown') {
